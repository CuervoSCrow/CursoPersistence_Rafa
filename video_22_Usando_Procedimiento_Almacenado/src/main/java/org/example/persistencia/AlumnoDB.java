...
public class AlumnoDB {
    ...
    public boolean promocionAlumnos_StoreProcedure(int idOrigen, int idDestino){
        try{
            StoredProcedureQuery query = em.createStoredProcedureQuery("promover_alumnos");
            ...
            em.getTransaction().commit();
            System.out.println(result1+" de los alumnos han pasado al siguiente curso y "
                            +result2+" han reprobado el cursp");
            return true;
        }catch(Exception e){
            em.getTransaction().rollback();
            System.out.println("Error: "+e.getMessage());
    ...
    public boolean promocionAlumnos_StoreProcedure_NamedStoreProcedureQuery(int idOrigen, int idDestino){
        try{
            StoredProcedureQuery query =    em.createNamedStoredProcedureQuery("Alumno.promocionarAlumnos");
